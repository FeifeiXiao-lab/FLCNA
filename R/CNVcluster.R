...
CNVcluster <-function(Y, cp, L) {
  ...
  EM[[1]] = gausianMixture(x=Y, cp, priors=priors1, L, st=st1)
  st2 = 2
  p2 = rep(1/st2, st2)
  sigma2 = rep(0.5, st2)
  mu2 = c(2, 4)
  priors2 = list(p = p2, mu = mu2, sigma = sigma2)
  EM[[2]] = gausianMixture(x=Y, cp, priors=priors2, L, st=st2)
  ...
  EM[[3]] = gausianMixture(x=Y, cp, priors=priors3, L, st=st3)
  for (i in 1:3) {
...
CNV.out <- function(mean.matrix, cutoff=0.5, L=100){
  CNVdata <- list()
  for (i in 1:nrow(mean.matrix)){
    cp.index <- 1+which(abs(diff(round(mean.matrix[i,],1)))>=cutoff)
    res <- CNVcluster(Y = mean.matrix[i,], cp=cp.index, L)
    CNVdata[[i]] <- data.frame(state=res$cnv.state, start=res$cnv.start, end=res$cnv.end)
  }
  return(CNVdata)
}
