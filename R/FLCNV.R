#' @name FLCNV
#' @aliases FLCNV
#' @title Simultaneous CNV detection and subclone identification using single cell DNA sequencing data.
#'
...
#' @param cutoff Cutoff value to further control the number of CNVs besed on mean matrix from FL model. Larger cutoff value, less CNVs.
#' @param L repeat times in the EM algorithm while outputing CNV data, defaults to 100
...
#' output <- FLCNV(K = c(1:2), lambda = c(0,1), y=y)
#' output
#'
#' @export
#' 
FLCNV <- function(tuning=NULL, K=NULL, lambda = NULL, y, N = 100, kms.iter = 100, kms.nstart = 100, 
                 adapt.kms = FALSE, eps.diff = 1e-5, eps.em = 1e-5, iter.LQA = 20, eps.LQA = 1e-5, 
                 cutoff=0.5, L=100, model.crit = 'gic'){
  ...
  for(j.tune in 1:dim(tuning)[1]){
    ...
    else if(lambda1 == 0)
    {
      ...
      for (t in 1:(N-1)) {
        ...
        mu[(t+1),,] = t(sapply(1:K1, FLCNV_LQA, K1, index.max=index.max, mu.t.all=mu[t,,], mu.no.penal=mu.no.penal, y=y, alpha=alpha.temp[(t+1),,], 
      ...
      #CNV output
      message(paste0("Start output CNV data for K=",K1,", lambda=",lambda1))
      CNVdata[[j.tune]] <- CNV.out(mean.matrix=mu.hat[[j.tune]], cutoff=cutoff, L=L)
