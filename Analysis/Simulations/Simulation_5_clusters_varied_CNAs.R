# Generate simulation data using spike-in strategy for 5 clusters and varied numbers of CNAs
#############################################################################################
...
setwd("E:\\10XGenomics\\breast_tissue_ABCDE_10k\\Simulation 5 clusters varied CNVs 5 states")
set.seed(2021)
random.Cell <- sample(1:5,ncol(rd2),replace = TRUE)
save(random.Cell, file="random.Cell.5clusters.RData")


Gen_simu <- function(rd2, nCluster, Percent){
  ...
  for (j in 1:nCluster){
    ...
    cpSuperShort
    cpSuperShort.list[[j]] <- cpSuperShort
